...
2. 采集为**事件驱动**：`SetWinEventHook` 监听窗口创建/销毁/显隐/聚焦/标题变化/前台切换，低层键盘/鼠标滚轮钩子监听打字/滚动；所有触发经 **200ms 静默合并**（200ms 内连续变化只写一次）。窗口级触发按标题/矩形去重并写入新窗口快照；内容级触发（打字/滚动）只落 OCR 文本（关联到最近一次窗口快照），不重复写窗口记录（文字可能已变而窗口未动）。另保留每 5 秒轮询兜底。按 **F12** 立即触发一次；右键托盘图标退出。
